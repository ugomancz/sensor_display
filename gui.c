...
/* Should the screen be cleared with next display_update() call */
...
int32_t touch_callback(uint32_t message, int32_t x, int32_t y) {
    /* Only handle touch screen releases */
    if (message == MSG_PTR_UP) {
        /* User chose not to use the found device */
...
void _init_display_menu() {
    ...
    Graphics_drawString(&g_context, "deg. C", -1, 125, 150, false);
...
void _update_display_menu() {
    if (ch_value.val != 0.0f) {
        const Graphics_Rectangle hide_temp = { .xMin = 68, .yMin = 150, .xMax = 124, .yMax = 175 };
        int8_t string_buffer[10] = { 0 };
        ...
        sprintf((char*) &string_buffer, "%0.2f", ch_value.val);
