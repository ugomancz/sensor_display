...
void _update_display_find_lookup(bool found) {
    ...
    Graphics_setFont(&g_context, &g_sFontCm24);
    ...
    draw_button(&g_context, &to_menu_button);
    if (found) {
        draw_button(&g_context, &find_accept_button);
        draw_button(&g_context, &find_reject_button);
    }
}
